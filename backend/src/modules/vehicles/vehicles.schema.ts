...
export const UpdateVehicleSchema = z
  .object({
    ...
    status: VehicleStatusEnum.refine((s) => s !== "ON_TRIP", {
      message: "ON_TRIP can only be set automatically by trip dispatch.",
    })
      // Integration note: IN_SHOP is also blocked here to prevent
      // KPI desync between Vehicle.status and open MaintenanceLog rows.
      // Only the maintenance module may set IN_SHOP automatically.
      .refine((s) => s !== "IN_SHOP", {
        message: "IN_SHOP can only be set automatically by opening a maintenance record.",
      })
      .optional(),
  })
  .strict();
...
export const VehicleQuerySchema = z.object({
  ...
  sort: z
    .enum(["name", "-name", "createdAt", "-createdAt", "odometerKm", "-odometerKm"])
    .default("-createdAt"),
